...
@time D, F, xcp, ycp = flow_routing2D(x, reverse(y), bed, surface.-bed, mask, mask*1)

# visu
F[.!mask] .= NaN
display(heatmap(xcp, ycp, log10.(F'), aspect_ratio=1, xlims=(xcp[1], xcp[end]), ylims=(ycp[1], ycp[end]), c=:viridis, framestyle=:box, title="log10 ||Flux||"))
